  ...
  try {
    ...
    if (fileType.includes('csv') || fileType.includes('spreadsheet')) {
      ...
      if (anthropicKey) {
        ...
        throw new Error('Anthropic API key not configured for PDF processing. Please add ANTHROPIC_API_KEY to environment variables.');
      }
    }
    } else {
...
async function parseCSV(fileData: Blob): Promise<Transaction[]> {
  ...
  const descIdx = findColumnIndex(headerLower, ['descricao', 'description', 'desc', 'historico']);
  const amountIdx = findColumnIndex(headerLower, ['valor', 'amount', 'value', 'total']);
  const categoryIdx = findColumnIndex(headerLower, ['categoria', 'category', 'tipo']);
...
// ============================================================================
async function extractWithAI(fileData: Blob, fileType: string, apiKey: string): Promise<Transaction[]> {
  // Check file size (max 5MB for vision API)
  const sizeMB = fileData.size / (1024 * 1024);
  if (sizeMB > 5) {
    throw new Error(`File too large for AI processing: ${sizeMB.toFixed(1)}MB (max 5MB)`);
  }
  ...
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a financial categorization AI. Categorize transactions into: Alimentação, Transporte, Saúde, Educação, Lazer, Casa, Vestuário, Outros.',
        },
        {
          role: 'user',
...
// ============================================================================
async function extractPDFWithClaude(fileData: Blob, apiKey: string): Promise<Transaction[]> {
